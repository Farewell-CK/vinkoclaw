...
graph TB
    ...
    subgraph Orchestrator["⚙️ Orchestrator (调度中心)"]
        Router[意图分类<br/>LLM + 关键词兜底]
        Policy[路由策略<br/>模板/角色/协作]
        Approval[审批引擎<br/>风险分级]
        API[REST API]
    end

    subgraph Storage["💾 SQLite 状态层"]
        Tasks[(tasks)]
        Approvals[(approvals)]
        Audit[(audit_events)]
    ...
    subgraph Runner["🏃 Task Runner (执行层)"]
        KB[Knowledge Base<br/>文档/代码检索]
        Runtime[Agent Runtime<br/>prompt + 推理]
        Tools[Tool Executor<br/>opencode/codex]
        Reflect[Reflection<br/>评分/置信度/风险]
    end

    subgraph Model["🤖 推理后端"]
        vLLM[vLLM<br/>Qwen 3.5-35B-A3B FP8]
        SGLang[SGLang / Ollama<br/>兜底]
        Fallback[Deterministic<br/>降级]
    end
...
sequenceDiagram
    autonumber
    participant Owner as Owner (飞书/控制台)
    participant Orch as Orchestrator
    participant SQLite as SQLite
    participant Runner as Task Runner
    participant KB as Knowledge Base
    participant Runtime as Agent Runtime
    participant Model as vLLM / SGLang
    participant Back as Owner 回执

    Owner->>Orch: 发送指令
    Orch->>Orch: 意图分类<br/>(LLM + 关键词兜底)
    Orch->>Orch: 路由到角色
    Orch->>SQLite: 写入任务 (queued)
    Runner->>SQLite: 领取任务 (running)
    Runner->>KB: 检索上下文<br/>(文档/代码/历史)
    KB-->>Runner: 返回上下文
    Runner->>Runtime: 拼接角色 prompt + 技能
    Runtime->>Model: 推理请求
    Model-->>Runtime: 推理结果
    Runtime->>Runtime: 生成 reflection<br/>(score/confidence/risks)
    Runtime->>SQLite: 写入 result + reflection (completed)
    Orch->>Back: 回执 (飞书卡片/控制台通知)
```
...
sequenceDiagram
    ...
    participant Orch as Orchestrator
    participant Collab as Collaboration Manager
    participant P as PM 实例
    participant F as Frontend 实例
    participant B as Backend 实例
    participant Q as QA 实例
    participant Agg as 聚合器
    participant Back as Owner 回执

    Owner->>Orch: "团队执行：做一个活动落地页"
    Orch->>Orch: 识别 collaboration 意图
    Orch->>Collab: 创建协作记录
    Collab->>Collab: 拆分子任务<br/>PM → Frontend → Backend → QA

    par 并行执行
        Collab->>P: PM 角色子任务
        P->>P: 需求拆解 + PRD
        P-->>Collab: timeline 事件 + 进度推送
    and
        Collab->>F: Frontend 角色子任务
        F->>F: 页面代码 + 样式
        F-->>Collab: timeline 事件 + 进度推送
    and
        Collab->>B: Backend 角色子任务
        B->>B: API 设计 + 服务实现
        B-->>Collab: timeline 事件 + 进度推送
    and
        Collab->>Q: QA 角色子任务
        Q->>Q: 测试矩阵 + 回归策略
        Q-->>Collab: timeline 事件 + 进度推送
    end

    Collab->>Collab: 执行阶段收敛<br/>检查所有角色完成状态
    Collab->>Agg: 触发聚合任务
    Agg->>Agg: 汇总各角色输出<br/>生成最终 deliverable
    Agg->>Agg: 生成 reflection
    Agg-->>Collab: 聚合结果
    Collab->>Collab: 协作状态 completed
    Collab->>Back: 回执 (汇总报告 + 各角色产出)
```
...
flowchart LR
    ...
    subgraph Parse["解析层"]
        OP[operator_action<br/>解析]
    end

    subgraph Risk["风险分级"]
        High{高风险?<br/>deploy/rm -rf/邮件发送}
        Med{中风险?<br/>安装/迁移}
        Low[低风险]
    end

    subgraph Approval["审批层"]
        OwnerApprove[Owner 审批<br/>飞书交互式卡片]
        CTOApprove[CTO 自动审批<br/>可配置]
    end

    subgraph Execute["执行层"]
        Config[配置变更<br/>skill/memory/通道]
        Audit[审计日志]
    end
