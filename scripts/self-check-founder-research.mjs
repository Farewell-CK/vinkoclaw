...
const PREFLIGHT_TIMEOUT_MS = Number(process.env.FOUNDER_RESEARCH_PREFLIGHT_TIMEOUT_MS ?? 120_000);

function emitHarnessMeta(patch) {
  process.stderr.write(`HARNESS_META ${JSON.stringify({ suite: "founder-research", ...patch })}\n`);
}
...
function fail(message, patch = {}) {
  ...
  process.stderr.write(`[self-check:founder-research] FAIL: ${message}\n`);
  process.exit(1);
}

function info(message) {
  process.stdout.write(`[self-check:founder-research] ${message}\n`);
}
...
async function runQueuePreflightDrain() {
  ...
  const exitCode = await runResetRuntimeDrain({
    ...
    timeoutMs: PREFLIGHT_TIMEOUT_MS,
    maxBusyTasks: 0,
    reason: `founder-research-selfcheck-${Date.now()}`
  });
  if (exitCode !== 0) {
  ...
  try {
    ...
    const created = await fetchJson("/api/messages", {
      ...
      body: JSON.stringify({
        ...
        requestedBy: `selfcheck-founder-research-${Date.now()}`,
        text: [
          "请帮我做一份 AI 编程助手市场调研报告。",
          "目标读者是创始人，重点看产品定位、价格带、差异化能力和进入机会。",
          "输出结构化报告，并给出下一步建议。"
        ].join("\n")
      })
    });
    ...
    if (created?.templateId !== "tpl-founder-research-report") {
      fail(`expected templateId tpl-founder-research-report, got ${String(created?.templateId ?? "")}`, {
        stage: "dispatch",
        regressionCategory: "template_mismatch"
      });
    }
    if (!Array.isArray(created?.taskIds) || created.taskIds.length !== 1) {
      fail(`expected exactly one research task, got ${JSON.stringify(created?.taskIds ?? null)}`, {
        stage: "dispatch",
    ...
    if (!task || task.id !== taskId) {
      fail(`research task ${taskId} could not be loaded`, {
        stage: "inspect",
