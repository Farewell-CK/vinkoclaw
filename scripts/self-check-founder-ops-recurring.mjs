...
const PREFLIGHT_TIMEOUT_MS = Number(process.env.FOUNDER_OPS_RECURRING_PREFLIGHT_TIMEOUT_MS ?? 120_000);

function emitHarnessMeta(patch) {
  process.stderr.write(`HARNESS_META ${JSON.stringify({ suite: "founder-ops-recurring", ...patch })}\n`);
}
...
function fail(message, patch = {}) {
  ...
  process.stderr.write(`[self-check:founder-ops-recurring] FAIL: ${message}\n`);
  process.exit(1);
}

function info(message) {
  process.stdout.write(`[self-check:founder-ops-recurring] ${message}\n`);
}
...
async function runQueuePreflightDrain() {
  ...
  const exitCode = await runResetRuntimeDrain({
    ...
    timeoutMs: PREFLIGHT_TIMEOUT_MS,
    maxBusyTasks: 0,
    reason: `founder-ops-recurring-selfcheck-${Date.now()}`
  });
  if (exitCode !== 0) {
...
async function main() {
  ...
  try {
    ...
    const requestedBy = `selfcheck-founder-ops-recurring-${Date.now()}`;
    ...
    let created = await postMessage(
      [
        "请帮我整理一份周期性运营执行清单。",
        "每周一早上整理上周用户反馈，每周三联系 5 个潜在客户，每周五输出下周重点动作。",
        "输出时要明确周期、触发规则、责任归属、完成信号和风险。"
      ].join("\n")
    );
    ...
    if (created?.type === "template_tasks_queued") {
      if (created?.templateId !== "tpl-founder-ops-recurring") {
        fail(`expected templateId tpl-founder-ops-recurring, got ${String(created?.templateId ?? "")}`, {
          stage: "dispatch",
          regressionCategory: "template_mismatch"
        });
      }
      if (!Array.isArray(created?.taskIds) || created.taskIds.length !== 1) {
        fail(`expected exactly one recurring ops task, got ${JSON.stringify(created?.taskIds ?? null)}`, {
          stage: "dispatch",
    ...
    if (!task || task.id !== taskId) {
      fail(`recurring ops task ${taskId} could not be loaded`, {
        stage: "inspect",
