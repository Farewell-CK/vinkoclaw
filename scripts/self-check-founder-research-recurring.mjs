...
const PREFLIGHT_TIMEOUT_MS = Number(process.env.FOUNDER_RESEARCH_RECURRING_PREFLIGHT_TIMEOUT_MS ?? 120_000);

function emitHarnessMeta(patch) {
  process.stderr.write(`HARNESS_META ${JSON.stringify({ suite: "founder-research-recurring", ...patch })}\n`);
}
...
function fail(message, patch = {}) {
  ...
  process.stderr.write(`[self-check:founder-research-recurring] FAIL: ${message}\n`);
  process.exit(1);
}

function info(message) {
  process.stdout.write(`[self-check:founder-research-recurring] ${message}\n`);
}
...
async function runQueuePreflightDrain() {
  ...
  const exitCode = await runResetRuntimeDrain({
    ...
    timeoutMs: PREFLIGHT_TIMEOUT_MS,
    maxBusyTasks: 0,
    reason: `founder-research-recurring-selfcheck-${Date.now()}`
  });
  if (exitCode !== 0) {
...
async function main() {
  ...
  try {
    ...
    const requestedBy = `selfcheck-founder-research-recurring-${Date.now()}`;
    ...
    let created = await postMessage(
      [
        "请帮我整理一份每周竞品跟踪调研清单。",
        "每周二固定跟踪 Cursor、Claude Code、OpenAI Codex 的产品更新、价格带和差异化能力。",
        "输出时要明确周期、跟踪对象、观察维度、输出结构、完成信号和建议动作。"
      ].join("\n")
    );
    ...
    if (created?.type === "template_tasks_queued") {
      if (created?.templateId !== "tpl-founder-research-recurring") {
        fail(`expected templateId tpl-founder-research-recurring, got ${String(created?.templateId ?? "")}`, {
          stage: "dispatch",
          regressionCategory: "template_mismatch"
        });
      }
      if (!Array.isArray(created?.taskIds) || created.taskIds.length !== 1) {
        fail(`expected exactly one recurring research task, got ${JSON.stringify(created?.taskIds ?? null)}`, {
          stage: "dispatch",
    ...
    if (!task || task.id !== taskId) {
      fail(`recurring research task ${taskId} could not be loaded`, {
        stage: "inspect",
        regressionCategory: "task_unreachable"
      });
    }
    if (task.roleId !== "research") {
      fail(`expected research role, got ${String(task.roleId ?? "")}`, {
        stage: "inspect",
