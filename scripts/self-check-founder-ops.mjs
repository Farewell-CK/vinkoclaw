...
const PREFLIGHT_TIMEOUT_MS = Number(process.env.FOUNDER_OPS_PREFLIGHT_TIMEOUT_MS ?? 120_000);

function emitHarnessMeta(patch) {
  process.stderr.write(`HARNESS_META ${JSON.stringify({ suite: "founder-ops", ...patch })}\n`);
}
...
function fail(message, patch = {}) {
  ...
  process.stderr.write(`[self-check:founder-ops] FAIL: ${message}\n`);
  process.exit(1);
}

function info(message) {
  process.stdout.write(`[self-check:founder-ops] ${message}\n`);
}
...
async function runQueuePreflightDrain() {
  ...
  const exitCode = await runResetRuntimeDrain({
    ...
    timeoutMs: PREFLIGHT_TIMEOUT_MS,
    maxBusyTasks: 0,
    reason: `founder-ops-selfcheck-${Date.now()}`
  });
  if (exitCode !== 0) {
  ...
  try {
    ...
    const created = await fetchJson("/api/messages", {
      ...
      body: JSON.stringify({
        ...
        requestedBy: `selfcheck-founder-ops-${Date.now()}`,
        text: [
          "请帮我整理接下来三天的运营跟进和待办提醒。",
          "周二给 5 个种子用户回访，周三汇总反馈，周四整理下周待办。",
          "输出成可执行清单，包含提醒时间、责任归属、风险和下一步。"
        ].join("\n")
      })
    });
    ...
    if (created?.templateId !== "tpl-founder-ops-followup") {
      fail(`expected templateId tpl-founder-ops-followup, got ${String(created?.templateId ?? "")}`, {
        stage: "dispatch",
        regressionCategory: "template_mismatch"
      });
    }
    if (!Array.isArray(created?.taskIds) || created.taskIds.length !== 1) {
      fail(`expected exactly one ops task, got ${JSON.stringify(created?.taskIds ?? null)}`, {
        stage: "dispatch",
    ...
    if (!task || task.id !== taskId) {
      fail(`ops task ${taskId} could not be loaded`, {
        stage: "inspect",
