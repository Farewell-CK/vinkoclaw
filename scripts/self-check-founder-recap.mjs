...
const PREFLIGHT_TIMEOUT_MS = Number(process.env.FOUNDER_RECAP_PREFLIGHT_TIMEOUT_MS ?? 120_000);

function emitHarnessMeta(patch) {
  process.stderr.write(`HARNESS_META ${JSON.stringify({ suite: "founder-recap", ...patch })}\n`);
}
...
function fail(message, patch = {}) {
  ...
  process.stderr.write(`[self-check:founder-recap] FAIL: ${message}\n`);
  process.exit(1);
}

function info(message) {
  process.stdout.write(`[self-check:founder-recap] ${message}\n`);
}
...
async function runQueuePreflightDrain() {
  ...
  const exitCode = await runResetRuntimeDrain({
    ...
    timeoutMs: PREFLIGHT_TIMEOUT_MS,
    maxBusyTasks: 0,
    reason: `founder-recap-selfcheck-${Date.now()}`
  });
  if (exitCode !== 0) {
  ...
  try {
    ...
    const created = await fetchJson("/api/messages", {
      ...
      body: JSON.stringify({
        ...
        requestedBy: `selfcheck-founder-recap-${Date.now()}`,
        text: [
          "请帮我整理一份这周的 founder weekly recap。",
          "已完成：上线登录页 MVP、接入飞书消息入口、补了 persona harness。",
          "阻塞：文档导出还不稳定；下周计划：补 implementation workflow coverage。"
        ].join("\n")
      })
    });
    ...
    if (created?.templateId !== "tpl-founder-weekly-recap") {
      fail(`expected templateId tpl-founder-weekly-recap, got ${String(created?.templateId ?? "")}`, {
        stage: "dispatch",
        regressionCategory: "template_mismatch"
      });
    }
    if (!Array.isArray(created?.taskIds) || created.taskIds.length !== 1) {
      fail(`expected exactly one recap task, got ${JSON.stringify(created?.taskIds ?? null)}`, {
        stage: "dispatch",
    ...
    if (!task || task.id !== taskId) {
      fail(`recap task ${taskId} could not be loaded`, {
        stage: "inspect",
